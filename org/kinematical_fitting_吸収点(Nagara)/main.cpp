...
double chi2(double m1)
{
	...
	err_theta1_phi1_rad = 1.74231*pow(10,-5);//*pow(10,-);//0;//1.74231*pow(10,-5);//0;
	err_theta2_phi2_rad = 7.87901*pow(10,-4);//*pow(10,-);//0;//7.87901*pow(10,-4);//0;
	err_theta3_phi3_rad = 5.28957*pow(10,-6);//*pow(10,-);//0;//5.28957*pow(10,-6);//0;
	...
	double new_err_BLL = sqrt(4*pow(0.006,2)+pow(new_err_m1,2));
	...
	printf("m_D       = %.3f\n",m1);
	printf("BLL       = %.3f\n",1115.683*2+3727.380-m1);
	printf("m_D(new)  = %.4f Å} %.4f\n",new_m1,new_err_m1);
	printf("BLL(new)  = %.4f Å} %.4f\n",1115.683*2+3727.380-new_m1,new_err_BLL);
...
int NumericalMinimization()
{
	...
	bm.SetFunction(func,5950.000,5955.000);
	bm.Minimize(5,0,0);
	...
	//cout << "chi(" << bm.XMinimum() << ") = " <<bm.FValMinimum() << endl;

	//printf("%.9f\n",B_xi_minimum);
	//printf("%.9f",chi_square_minimum);
