...
/*
double P(double Mass,double Range,int Z,int A,int S)
...
while(dKE>0.00005)
{
	...
	if((P_sub(Mass,KE,Z)>=Range && Range>=R0) || (P_sub(Mass,KE,Z)<=Range && Range<=R0))

    {
        dKE = dKE/10.0;
		KE = KE0+(KE-KE0)*(Range-R0)/(P_sub(Mass,KE,Z)-R0); 
...
double P_sub(double Mass,double KE,int Z)
{
...
double rate_grobal2,Rp_grobal2,Rs_grobal2;


double P(double Mass,double Range,int Z,int A,int S)//(double Mass,double Range,int Z,double D,double r)
...
while(dKE>0.00005)
{
	...
	double return_R = P_sub(Mass,KE,Z);//(Mass,KE,Z,D,r);
...

    {
		...
		R0 = P_sub(Mass,KE,Z);// function1(Mass,KE,Z,D,r);
...
double LMp2 = log(938.272);

double Rs_function4(double LR)
{
