...

{
  ...
  int Z1,Z2,Z3,Z4,A1,A2,A3,A4,S1,S2,S3,S4;//粒子の原子番号、質量数、ストレンジ数（S=3は、中間子など）
  int Z1_1,Z2_1,Z3_1,Z4_1,A1_1,A2_1,A3_1,A4_1,S1_1,S2_1,S3_1,S4_1;//反応式を作成するための制限を作るうえで使用する原子番号と質量数とストレンジ数
  double Mass1,Mass2,Mass3,Mass4;//粒子の質量
	  ...
	  for(Z2=1;Z2<=8;Z2++){
		...
		for(A3=1;A3<=19;A3++){     //すべての粒子に対してRange,theta,phiを当てはめていく。
		  for(S4=0;S4<=1;S4++){
			...
			double Q = (Estimated_Mass - Mass1 - Mass2 - Mass3 - Mass4);//Q値



			if((Mass1 <= 0.)||(Mass2 <= 0.)||(Mass3 <= 0.)||(Mass4 <= 0.))//粒子の質量が０になってしまうところは飛ばす。
			  {
			    continue;
			  }
			...
			if((ZZ!=Z1_1+Z2_1+Z3_1+Z4_1)||(AA!=A1_1+A2_1+A3_1+A4_1))
			  {
			    continue;
			  }
			...
			double p4x = -1*(p1x + p2x + p3x);
			double p4y = -1*(p1y + p2y + p3y);
			double p4z = -1*(p1z + p2z + p3z);//中性粒子の運動量成分
			...
			double cosF = (p4x*p3x+p4y*p3y+p4z*p3z)/((sqrt(p4x*p4x+p4y*p4y+p4z*p4z))*(sqrt(p3x*p3x+p3y*p3y+p3z*p3z)));





			double errp1x,errp1y,errp1z,errp2x,errp2y,errp2z,errp3x,errp3y,errp3z,errp4x,errp4y,errp4z;//各粒子の運動量成分の誤差
			double errp1,errp2,errp3,errp4;//各粒子の運動量の誤差
			double errcosA,errcosB,errcosC,errcosD,errcosE,errcosF;//各粒子の運動量ベクトルの角度をA,B,Cとした時のcos成分の誤差
			...
			errp4x = sqrt(pow(errp1x,2)+pow(errp2x,2)+pow(errp3x,2));
			errp4y = sqrt(pow(errp1y,2)+pow(errp2y,2)+pow(errp3y,2));
			errp4z = sqrt(pow(errp1z,2)+pow(errp2z,2)+pow(errp3z,2));
			...
			errcosF = sqrt(pow((p3x*sqrt((pow(p4x,2)+pow(p4y,2)+pow(p4z,2)))-p4x*(p4x*p3x+p4y*p3y+p4z*p3z)*pow(pow(p4x,2)+pow(p4y,2)+pow(p4z,2),-0.5))
					   /((pow(p4x,2)+pow(p4y,2)+pow(p4z,2))*sqrt(pow(p3x,2)+pow(p3y,2)+pow(p3z,2))),2)*pow(errp4x,2)+
				       pow((p4x*sqrt((pow(p3x,2)+pow(p3y,2)+pow(p3z,2)))-p3x*(p4x*p3x+p4y*p3y+p4z*p3z)*pow(pow(p3x,2)+pow(p3y,2)+pow(p3z,2),-0.5))
					   /((pow(p3x,2)+pow(p3y,2)+pow(p3z,2))*sqrt(pow(p4x,2)+pow(p4y,2)+pow(p4z,2))),2)*pow(errp3x,2)+
				       pow((p3y*sqrt((pow(p4x,2)+pow(p4y,2)+pow(p4z,2)))-p4y*(p4x*p3x+p4y*p3y+p4z*p3z)*pow(pow(p4x,2)+pow(p4y,2)+pow(p4z,2),-0.5))
					   /((pow(p4x,2)+pow(p4y,2)+pow(p4z,2))*sqrt(pow(p3x,2)+pow(p3y,2)+pow(p3z,2))),2)*pow(errp4y,2)+
				       pow((p4y*sqrt((pow(p3x,2)+pow(p3y,2)+pow(p3z,2)))-p3y*(p4x*p3x+p4y*p3y+p4z*p3z)*pow(pow(p3x,2)+pow(p3y,2)+pow(p3z,2),-0.5))
					   /((pow(p3x,2)+pow(p3y,2)+pow(p3z,2))*sqrt(pow(p4x,2)+pow(p4y,2)+pow(p4z,2))),2)*pow(errp3y,2)+
				       pow((p3z*sqrt((pow(p4x,2)+pow(p4y,2)+pow(p4z,2)))-p4z*(p4x*p3x+p4y*p3y+p4z*p3z)*pow(pow(p4x,2)+pow(p4y,2)+pow(p4z,2),-0.5))
					   /((pow(p4x,2)+pow(p4y,2)+pow(p4z,2))*sqrt(pow(p3x,2)+pow(p3y,2)+pow(p3z,2))),2)*pow(errp4z,2)+
				       pow((p4z*sqrt((pow(p3x,2)+pow(p3y,2)+pow(p3z,2)))-p3z*(p4x*p3x+p4y*p3y+p4z*p3z)*pow(pow(p3x,2)+pow(p3y,2)+pow(p3z,2),-0.5))
					   /((pow(p3x,2)+pow(p3y,2)+pow(p3z,2))*sqrt(pow(p4x,2)+pow(p4y,2)+pow(p4z,2))),2)*pow(errp3z,2));


			//親粒子の質量
			double Caluculated_Mass;

			Caluculated_Mass = sqrt(pow(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))+sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)),2)
						-(pow(momentum1,2)+pow(momentum2,2)+pow(momentum3,2)+pow(momentum4,2)+2*momentum1*momentum2*cosA+2*momentum2*momentum3*cosB+2*momentum1*momentum3*cosC+
						  2*momentum1*momentum4*cosD+2*momentum2*momentum4*cosE+2*momentum3*momentum4*cosF));
			...
			double total_kinetic_energy = kinetic_energy1 + kinetic_energy2 + kinetic_energy3 + kinetic_energy4;//娘粒子の運動エネルギーの合計
			double err_total_kinetic_energy = sqrt(pow(err_kinetic_energy1,2) + pow(err_kinetic_energy2,2) + pow(err_kinetic_energy3,2)+pow(err_kinetic_energy4,2));//娘粒子の運動エネルギーの合計の誤差
			...
			//計算された親粒子の質量の誤差
			double err_Caluculated_Mass = sqrt(pow(0.5*pow(pow(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))+sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)),2)
								       -(pow(momentum1,2)+pow(momentum2,2)+pow(momentum3,2)+pow(momentum4,2)+2*momentum1*momentum2*cosA+2*momentum2*momentum3*cosB+2*momentum1*momentum3*cosC+2*momentum1*momentum4*cosD+
									 2*momentum2*momentum4*cosE+2*momentum3*momentum4*cosF),-0.5)*(2*momentum1*pow(pow(Mass1,2)+pow(momentum1,2),-0.5)*(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))
																							    +sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)))-2*(momentum1+momentum2*cosA+momentum3*cosC+momentum4*cosD)),2)*pow(errp1,2)+
							   pow(0.5*pow(pow(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))+sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)),2)
								       -(pow(momentum1,2)+pow(momentum2,2)+pow(momentum3,2)+pow(momentum4,2)+2*momentum1*momentum2*cosA+2*momentum2*momentum3*cosB+2*momentum1*momentum3*cosC+2*momentum1*momentum4*cosD+
									 2*momentum2*momentum4*cosE+2*momentum3*momentum4*cosF),-0.5)*(2*momentum2*pow(pow(Mass2,2)+pow(momentum2,2),-0.5)*(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))
																							    +sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)))-2*(momentum2+momentum3*cosB+momentum4*cosE+momentum1*cosA)),2)*pow(errp2,2)+
							   pow(0.5*pow(pow(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))+sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)),2)
								       -(pow(momentum1,2)+pow(momentum2,2)+pow(momentum3,2)+pow(momentum4,2)+2*momentum1*momentum2*cosA+2*momentum2*momentum3*cosB+2*momentum1*momentum3*cosC+2*momentum1*momentum4*cosD+
									 2*momentum2*momentum4*cosE+2*momentum3*momentum4*cosF),-0.5)*(2*momentum3*pow(pow(Mass3,2)+pow(momentum3,2),-0.5)*(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))
																							    +sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)))-2*(momentum3+momentum2*cosB+momentum1*cosC+momentum4*cosF)),2)*pow(errp3,2)+
							   pow(0.5*pow(pow(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))+sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)),2)
								       -(pow(momentum1,2)+pow(momentum2,2)+pow(momentum3,2)+pow(momentum4,2)+2*momentum1*momentum2*cosA+2*momentum2*momentum3*cosB+2*momentum1*momentum3*cosC+2*momentum1*momentum4*cosD+
									 2*momentum2*momentum4*cosE+2*momentum3*momentum4*cosF),-0.5)*(2*momentum4*pow(pow(Mass4,2)+pow(momentum4,2),-0.5)*(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))
																							    +sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)))-2*(momentum4+momentum1*cosD+momentum2*cosE+momentum3*cosF)),2)*pow(errp4,2)+
							   pow(pow(pow(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))+sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)),2)
								   -(pow(momentum1,2)+pow(momentum2,2)+pow(momentum3,2)+pow(momentum4,2)+2*momentum1*momentum2*cosA+2*momentum2*momentum3*cosB+2*momentum1*momentum3*cosC+2*momentum1*momentum4*cosD+
								     2*momentum2*momentum4*cosE+2*momentum3*momentum4*cosF),-0.5)*momentum1*momentum2,2)*pow(errcosA,2)+
							   pow(pow(pow(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))+sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)),2)
								   -(pow(momentum1,2)+pow(momentum2,2)+pow(momentum3,2)+pow(momentum4,2)+2*momentum1*momentum2*cosA+2*momentum2*momentum3*cosB+2*momentum1*momentum3*cosC+2*momentum1*momentum4*cosD+
								     2*momentum2*momentum4*cosE+2*momentum3*momentum4*cosF),-0.5)*momentum2*momentum3,2)*pow(errcosB,2)+
							   pow(pow(pow(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))+sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)),2)
								   -(pow(momentum1,2)+pow(momentum2,2)+pow(momentum3,2)+pow(momentum4,2)+2*momentum1*momentum2*cosA+2*momentum2*momentum3*cosB+2*momentum1*momentum3*cosC+2*momentum1*momentum4*cosD+
								     2*momentum2*momentum4*cosE+2*momentum3*momentum4*cosF),-0.5)*momentum1*momentum3,2)*pow(errcosC,2)+
							   pow(pow(pow(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))+sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)),2)
								   -(pow(momentum1,2)+pow(momentum2,2)+pow(momentum3,2)+pow(momentum4,2)+2*momentum1*momentum2*cosA+2*momentum2*momentum3*cosB+2*momentum1*momentum3*cosC+2*momentum1*momentum4*cosD+
								     2*momentum2*momentum4*cosE+2*momentum3*momentum4*cosF),-0.5)*momentum1*momentum4,2)*pow(errcosD,2)+
							   pow(pow(pow(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))+sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)),2)
								   -(pow(momentum1,2)+pow(momentum2,2)+pow(momentum3,2)+pow(momentum4,2)+2*momentum1*momentum2*cosA+2*momentum2*momentum3*cosB+2*momentum1*momentum3*cosC+2*momentum1*momentum4*cosD+
								     2*momentum2*momentum4*cosE+2*momentum3*momentum4*cosF),-0.5)*momentum2*momentum4,2)*pow(errcosE,2)+
							   pow(pow(pow(sqrt(pow(Mass1,2)+pow(momentum1,2))+sqrt(pow(Mass2,2)+pow(momentum2,2))+sqrt(pow(Mass3,2)+pow(momentum3,2))+sqrt(pow(Mass4,2)+pow(momentum4,2)),2)
								   -(pow(momentum1,2)+pow(momentum2,2)+pow(momentum3,2)+pow(momentum4,2)+2*momentum1*momentum2*cosA+2*momentum2*momentum3*cosB+2*momentum1*momentum3*cosC+2*momentum1*momentum4*cosD+
								     2*momentum2*momentum4*cosE+2*momentum3*momentum4*cosF),-0.5)*momentum3*momentum4,2)*pow(errcosF,2));
			...
			if((Mass_gap <= err_Caluculated_Mass*3)&&(QQ <= err_total_kinetic_energy*3))//条件の合う崩壊過程のものだけresult.txtに書き出す
			  {
			    ...
			    fprintf(file1,"         反応式　　　\n");
			    fprintf(file1,"          %s  →  %s  +  %s  +  %s  +  %s  \n\n",sort[SS][AA-1][ZZ-1],sort[S1][A1-1][Z1-1],sort[S2][A2-1][Z2-1],sort[S3][A3-1][Z3-1],sort2[S4][A4-1][Z4]);
			  ...
			  //if((Mass_gap > err_Caluculated_Mass*3)||(QQ > err_total_kinetic_energy*3))//条件の合う崩壊過程のものだけresult.txtに書き出す
			  {
			    ...
			    fprintf(file1,"ーーーーーーーーーーーーーーーー－－－－－－－－－－－－－－ーーー－\n\n");
			    fprintf(file1,"         反応式　　　\n");
			    fprintf(file1,"          %s  →  %s  +  %s  +  %s  +  %s  \n\n",sort[SS][AA-1][ZZ-1],sort[S1][A1-1][Z1-1],sort[S2][A2-1][Z2-1],sort[S3][A3-1][Z3-1],sort2[S4][A4-1][Z4]);
			    fprintf(file1,"p1 = %9.3f ± %9.3f      P1x = %9.3f ± %9.3f\n",momentum1,errp1,p1x,errp1x);
			    fprintf(file1,"                                 p1y = %9.3f ± %9.3f\n",p1y,errp1y);
			    ...
			    fprintf(file1,"                    Q = %9.3f\n",Q);
			    fprintf(file1," Total_Kinetic_Energy = %9.3f ± %9.3f\n\n",total_kinetic_energy,err_total_kinetic_energy);
			    fprintf(file1,"ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー\n\n\n\n\n");
			    fclose(file1);
			  }
