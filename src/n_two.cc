	      ...
	      double errphi1, double errphi2 )
{
  ...
  int Z1,Z2,Z4,A1,A2,A4,S1,S2,S4;//粒子の原子番号、質量数、ストレンジ数（S=3は、中間子など）
  int Z1_1,Z2_1,Z4_1,A1_1,A2_1,A4_1,S1_1,S2_1,S4_1;//反応式を作成するための制限を作るうえで使用する原子番号と質量数とストレンジ数
  double Mass1,Mass2,Mass4;//粒子の質量
	  ...
	  for(Z2=1;Z2<=8;Z2++){
		...
		for(Z4=0;Z4<=3;Z4++){
		  for(A4=1;A4<=3;A4++){
		    ...
		    double Q = (Estimated_Mass - Mass1 - Mass2 - Mass4);//Q値

		    if((Mass1 <= 0.)||(Mass2 <= 0.)||(Mass4 <= 0.))//粒子の質量が０になってしまうところは飛ばす。
		      {
			continue;
		      }
		    ...
		    double p4z = -1*(p1z + p2z);//中性粒子の運動量成分
		    ...
		    double errp1x,errp1y,errp1z,errp2x,errp2y,errp2z,errp4x,errp4y,errp4z;//各粒子の運動量成分の誤差
		    double errp1,errp2,errp4;//各粒子の運動量の誤差
		    double errcosA,errcosD,errcosE;//各粒子の運動量ベクトルの角度をA,B,Cとした時のcos成分の誤差
		    ...
		    double total_kinetic_energy = kinetic_energy1 + kinetic_energy2 + kinetic_energy4;//娘粒子の運動エネルギーの合計
		    double err_total_kinetic_energy = sqrt(pow(err_kinetic_energy1,2) + pow(err_kinetic_energy2,2) + pow(err_kinetic_energy4,2)+pow(err_kinetic_energy4,2));//娘粒子の運動エネルギーの合計の誤差
			...
			fprintf(file1,"ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーー\n\n");
			fprintf(file1,"          反応式　　　\n");
			fprintf(file1,"          %s  →  %s  +  %s  +  %s  \n\n\n",sort[SS][AA-1][ZZ-1],sort[S1][A1-1][Z1-1],sort[S2][A2-1][Z2-1],sort2[S4][A4-1][Z4]);
			fprintf(file1,"p1 = %9.3f ± %9.3f      P1x = %9.3f ± %9.3f\n",momentum1,errp1,p1x,errp1x);
			fprintf(file1,"                                 p1y = %9.3f ± %9.3f\n",p1y,errp1y);
